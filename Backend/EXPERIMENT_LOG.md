...
**(2026-08-14 결정)** 이 트리거는 별도의 **WPF 신호**로 구현하기로 했다 — 백엔드 API(RegisterProduct 등)가
아니라, 작업자가 실물 투입을 흉내내는 조작 화면에서 "물건 올려두기" 신호를 보내는 방식. Serial 채번/등록은
여전히 백엔드(MES)가 하고, "재실 감지"만 이 WPF 쪽 신호로 대체한다 — 공장계획.md §2.2의 역할 분리
(PLC/물리 계층이 재실을 감지, MES는 Serial만 채번)와 일치하는 선택이다.
